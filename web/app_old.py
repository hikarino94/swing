...
def run(cmd_name: str):
    ...
    if cmd_name == "fetch_quotes":
        cmd = "python3 src/api/daily_quotes.py"
        if form.get("start"):
        ...
        cmd = "python3 src/api/listed_info.py"
    elif cmd_name == "statements":
        cmd = f"python3 src/api/statements.py {form.get('mode', '1')}"
        if form.get("start"):
            cmd += f" --start {form['start']}"
        if form.get("end"):
            cmd += f" --end {form['end']}"
        ...
        cmd = (
            f"python3 src/analysis/screen_statements.py --lookback {form.get('lookback')} "
            f"--recent {form.get('recent')}"
        )
        if form.get("as_of"):
            cmd += f" --as-of {form['as_of']}"
    elif cmd_name == "screen_tech":
        cmd = f"python3 src/analysis/screen_technical.py {form.get('cmd', 'indicators')}"
        if form.get("as_of"):
            cmd += f" --as-of {form['as_of']}"
        ...
        cmd = (
            f"python3 src/analysis/screen_ml.py screen --top {form.get('top', '30')} "
        ...
        cmd = (
            f"python3 src/backtest/backtest_statements.py --hold {form.get('hold')} "
        ...
        cmd = (
            f"python3 src/backtest/backtest_technical.py --start {form.get('start')} "
        ...
        cmd = "python3 tools/update_idtoken.py"
        if form.get("mail"):
            cmd += f" --mail {form['mail']}"
        if form.get("password"):
            cmd += f" --password {form['password']}"
    elif cmd_name == "db_summary":
        cmd = "python3 data/db/db_summary.py"
    elif cmd_name == "list_signals":
        cmd = f"python3 data/db/list_signals.py {form.get('kind')} --limit {form.get('limit')}"
        if form.get("start"):
            cmd += f" --start {form['start']}"
        if form.get("end"):
            cmd += f" --end {form['end']}"
    elif cmd_name == "analyze_json":
        cmd = "python3 src/backtest/analyze_backtest_json.py"
