...
"1911","住友林",300,,1455,1435,436500,430500,-6000
"7267","ホンダ",100,,1476,1443,147600,144300,-3300
"8306","三菱ＵＦＪ",200,,970,2015.5,194000,403100,+209100
...
銘柄コード,銘柄名称,保有株数,売却注文中,取得単価,現在値,取得金額,評価額,評価損益
"1814","大末建",100,,1607,2211,160700,221100,+60400
"7272","ヤマハ発",100,,1218,1083.5,121800,108350,-13450
...
銘柄コード,銘柄名称,保有株数,売却注文中,取得単価,現在値,取得金額,評価額,評価損益
"2914","ＪＴ",35,,3155,4218,110425,147630,+37205
"9432","ＮＴＴ",500,,163,153.9,81500,76950,-4550
...
"三菱ＵＦＪ−ｅＭＡＸＩＳ　Ｓｌｉｍ　米国株式（Ｓ＆Ｐ５００）",283135口,,31081,32551,880011,921632,+41621,再投資
"三菱ＵＦＪ−ｅＭＡＸＩＳ　Ｓｌｉｍ　全世界株式（オール・カントリー）",152067口,,25647,27477,390006,417834,+27828,再投資
"""
        ...
        assert result[2]["code"] == "8306"
        assert result[2]["account_type"] == "特定"

        # NISA口座の銘柄
        assert result[3]["code"] == "1814"
        assert result[3]["name"] == "大末建"
        assert result[3]["account_type"] == "NISA"
        assert result[3]["quantity"] == 100
        assert result[3]["profit_loss"] == 60400

        assert result[4]["code"] == "7272"
        assert result[4]["account_type"] == "NISA"

        # 旧NISA口座の銘柄
        assert result[5]["code"] == "2914"
        assert result[5]["account_type"] == "旧NISA"
        assert result[5]["quantity"] == 35

        assert result[6]["code"] == "9432"
        assert result[6]["account_type"] == "旧NISA"
        assert result[6]["quantity"] == 500

        # 投資信託はスキップされるため、以下のアサーションは削除
