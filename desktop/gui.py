...
def build_fetch_quotes_tab(nb, output):
    ...
    def _run():
        cmd = "python src/api/daily_quotes.py"
        if start_var.get():
...
def build_listed_info_tab(nb, output):
    ...
    def _run():
        cmd = "python src/api/listed_info.py"
...
def build_statements_tab(nb, output):
    ...
    def _run():
        cmd = f"python src/api/statements.py {mode_var.get()}"
        if start_var.get():
            cmd += f" --start {start_var.get()}"
        if end_var.get():
            cmd += f" --end {end_var.get()}"
...
def build_screen_fund_tab(nb, output):
    ...
    def _run():
        cmd = f"python src/analysis/screen_statements.py --lookback {lookback.get()} --recent {recent.get()}"
        if as_of.get():
...
def build_screen_tech_tab(nb, output):
    ...
    def _run():
        cmd = f"python src/analysis/screen_technical.py {cmd_var.get()}"
        if asof_var.get():
...
def build_backtest_stmt_tab(nb, output):
    ...
    def _run():
        ...
        xlsx.set(path)
        cmd = (
            f"python src/backtest/backtest_statements.py --hold {hold.get()} "
...
def build_backtest_tech_tab(nb, output):
    ...
    def _run():
        ...
        out.set(path)
        cmd = (
            f"python src/backtest/backtest_technical.py --start {start_var.get()} "
...
def build_backtest_ml_tab(nb, output):
    ...
    def _run():
        ...
        out.set(path)
        cmd = (
            f"python src/backtest/backtest_ml.py --start {start_var.get()} --top {top_var.get()} "
            f"--capital {cap.get()} --outfile {path}"
        )
        if end_var.get():
            cmd += f" --end {end_var.get()}"
...
def build_update_token_tab(nb, output):
    ...
    def _run():
        cmd = "python tools/update_idtoken.py"
        if mail.get():
            cmd += f" --mail {mail.get()}"
        if pwd.get():
...
def build_db_summary_tab(nb, output):
    ...
    def _run():
        cmd = "python data/db/db_summary.py"
        run_command(cmd, output)
...
def build_analyze_json_tab(nb, output):
    ...
    def _run():
        ...
        cmd = "python src/backtest/analyze_backtest_json.py " + " ".join(files)
        if show_var.get():
            cmd += " --show-trades"
        if side_var.get() != "all":
...
def build_signals_tab(nb, output):
    ...
    def _run():
        cmd = f"python data/db/list_signals.py {kind_var.get()} " f"--limit {limit_var.get()}"
        if start_var.get():
            cmd += f" --start {start_var.get()}"
        if end_var.get():
            cmd += f" --end {end_var.get()}"
...
def build_results_tab(nb):
    ...
    ttk.Button(frame, text="開く", command=open_selected).pack(anchor="e", padx=5, pady=(0, 5))
...
def build_screen_ml_tab(nb: ttk.Notebook, output: tk.Text):
    ...
    ttk.Checkbutton(arg, text="強制再学習", variable=retrain_var).grid(row=0, column=4, padx=(10, 0))

    # ── コマンド実行 ──────────────────────────────────────────────
    def _run():
        cmd = "python src/analysis/screen_ml.py screen " f"--top {top_var.get()} --lookback {lookback_var.get()}"
        if retrain_var.get():
            cmd += " --retrain"
        run_command(cmd, output)
