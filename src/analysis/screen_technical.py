...
DB_PATH = (Path(__file__).resolve().parents[2] / "data" / "db" / "stock.db").as_posix()
...
def compute_indicators(df):
    ...
    flags = pd.DataFrame(
        {
            "signal_ma": ((sma10 > sma20) & (sma20 > sma50) & (slope10 > 0) & (slope20 > 0) & (slope50 > 0)).astype(
                int
            ),
            ...
            "signal_ma_short": (
                (sma50 > sma20) & (sma20 > sma10) & (slope10 < 0) & (slope20 < 0) & (slope50 < 0)
            ).astype(int),
    ...
    flags["signals_count"] = flags[list(WEIGHTS)].mul(pd.Series(WEIGHTS)).sum(axis=1).astype(int)
    SHORT_WEIGHTS = {
        "signal_ma_short": 2,
        "signal_bb_short": 2,
        "signal_rsi_short": 1,
    ...
    flags["signals_short_count"] = flags[list(SHORT_WEIGHTS)].mul(pd.Series(SHORT_WEIGHTS)).sum(axis=1).astype(int)
...
def run_indicators(conn, as_of=None):
    ...
    if cnt == 0:
        ...
        return
    start = (datetime.strptime(as_of, "%Y-%m-%d") - timedelta(days=PRICE_LOOKBACK_DAYS)).strftime("%Y-%m-%d")
    ...
    all_flags = df_price.groupby("code", group_keys=False).apply(_calc).reset_index(drop=True)
    ...
    hist = pd.read_sql(
        "SELECT DISTINCT code FROM technical_indicators " "WHERE signal_date>=? AND signal_date<? AND signals_count>=?",
        conn,
    ...
    today_flags = today_flags[(today_flags["signals_oversold"] == 0) & (~today_flags["code"].isin(hist_short_codes))]
...
def screen_signals(conn, as_of=None):
    ...
    df = pd.read_sql(
        "SELECT * FROM technical_indicators " "WHERE signal_date=? AND (signals_count>=? OR signals_short_count>=?)",
        conn,
