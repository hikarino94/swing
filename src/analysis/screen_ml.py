...
Usage::
    ...
    # モデル学習（直近 3 年）
    python screen_ml.py train  --db ./data/db/stock.db --lookback 1095

    # 予測して上位 30 銘柄を出力（必要なら --retrain）
    python screen_ml.py screen --db ./data/db/stock.db --top 30
"""
...
def cli():
    ...
    p.add_argument("--db", default="./data/db/stock.db", help="SQLite DB path")
    ...
    out = feat_df.sort_values("prob_up30d", ascending=False).head(args.top).loc[:, ["code", "prob_up30d"]]
