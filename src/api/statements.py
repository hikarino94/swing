...
from src.utils.common import parse_date_string
from src.utils.db_utils import DatabaseManager, get_db_manager
from src.utils.exceptions import APIError, DatabaseError, DataError
...
class StatementsService:
    ...
    def update_by_codes(self, max_workers: int = 5) -> dict[str, Any]:
        ...
        try:
            ...
            if not codes:
                return {"status": "success", "message": "有効な銘柄コードが見つかりません", "records_updated": 0}

            # データ取得
            statements = self.fetcher.fetch_multiple_codes(codes, max_workers)

            if not statements:
                return {"status": "success", "message": "取得データが空です", "records_updated": 0}
            ...
            return {"status": "error", "error": f"予期しないエラーが発生しました: {e}", "error_type": "UnexpectedError"}
    ...
    def update_by_date(self, target_date: Optional[Union[str, date]] = None) -> dict[str, Any]:
        ...
        try:
            ...
            if not statements:
                return {"status": "success", "message": f"日付 {target_date} のデータが見つかりません", "records_updated": 0}
    ...
    def update_by_date_range(self, start_date: Union[str, date], end_date: Union[str, date]) -> dict[str, Any]:
        """期間指定で財務諸表データを更新
...
def create_statements_parser():
    ...
    parser.add_argument("mode", choices=["1", "2"], help="1: 銘柄ごとに一括取得、2: 日付または期間で取得")
