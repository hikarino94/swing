...
def run_command(cmd: str, timeout: Optional[int] = None, check: bool = True) -> Tuple[str, str, int]:
    """コマンド実行の共通ロジック
    ...
    try:
        proc = subprocess.run(shlex.split(cmd), capture_output=True, text=True, timeout=timeout, check=check)
...
def generate_timestamped_filename(
    base_name: str, extension: str, directory: Optional[Path] = None, timestamp_format: str = "%Y%m%d_%H%M%S"
) -> Path:
    """タイムスタンプ付きファイル名を生成
...
def save_dataframe(df: pd.DataFrame, filepath: Path, format: str = "xlsx", **kwargs) -> None:
    """DataFrameを指定形式で保存
...
def parse_date_range(
    date_from: Optional[str] = None, date_to: Optional[str] = None, default_days: int = 30
) -> Tuple[date, date]:
    """日付範囲を解析
