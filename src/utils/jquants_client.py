...
class JQuantsClient:
    ...
    def get(self, endpoint: str, params: Optional[Dict[str, Any]] = None, timeout: int = 60) -> Dict[str, Any]:
        """API呼び出しの共通実装
    ...
    def get_daily_quotes(self, code: Optional[str] = None, target_date: Optional[date] = None) -> List[Dict[str, Any]]:
        """日次株価情報を取得
        ...
        response = self.get("prices/daily_quotes", params)
        return response.get("daily_quotes", [])

    def get_statements(
        self, code: Optional[str] = None, date_from: Optional[date] = None, date_to: Optional[date] = None
    ) -> List[Dict[str, Any]]:
        """財務諸表データを取得
    ...
    def get_listed_info(self, code: Optional[str] = None, date_str: Optional[str] = None) -> List[Dict[str, Any]]:
        """上場銘柄情報を取得
    ...
    def refresh_token(self, email: str, password: str) -> str:
        ...
        try:
            response = requests.post(url, json={"mailaddress": email, "password": password}, timeout=30)
