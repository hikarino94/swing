...
from .common import generate_timestamped_filename
...
class PriceDataProvider:
    ...
    def get_price_data(
        self, codes: List[str], start_date: date, end_date: date, use_adjusted: bool = True
    ) -> pd.DataFrame:
        """株価データを取得
        ...
        price_cols = (
            ["adj_open", "adj_high", "adj_low", "adj_close"] if use_adjusted else ["open", "high", "low", "close"]
        )
    ...
    def get_price_at_date(
        self, code: str, target_date: date, price_type: str = "open", search_days: int = 5
    ) -> Optional[float]:
        """指定日の株価を取得（前後の営業日も検索）
...
class SignalProvider:
    ...
    def get_fundamental_signals(
        self, start_date: date, end_date: date, additional_filters: Optional[Dict[str, Any]] = None
    ) -> pd.DataFrame:
        """ファンダメンタルシグナルを取得
    ...
    def get_technical_signals(
        self, start_date: date, end_date: date, signal_types: Optional[List[str]] = None
    ) -> pd.DataFrame:
        """テクニカルシグナルを取得
...
class BacktestEngine:
    ...
    def __init__(
        self, price_provider: Optional[PriceDataProvider] = None, signal_provider: Optional[SignalProvider] = None
    ):
        """
        Args:
    ...
    def run_backtest(self, signals_df: pd.DataFrame, config: BacktestConfig) -> BacktestResult:
        """バックテストを実行
        ...
        for _, signal in signals_sorted.iterrows():
            ...
            # エントリー価格を取得
            entry_price = self._get_price_from_pivot(price_pivot["open"], code, entry_date, search_days=5)
        ...
        self._close_positions(current_positions, price_pivot, config, trades)
    ...
    def _get_price_from_pivot(
        self, price_pivot: pd.DataFrame, code: str, target_date: date, search_days: int = 5
    ) -> Optional[float]:
        """ピボットテーブルから価格を取得"""
    ...
    def _close_positions(
        ...
        for code, position in positions.items():
            ...
            # 終了価格を取得
            exit_price = self._get_price_from_pivot(price_pivot["open"], code, exit_date, search_days=5)
    ...
    def _calculate_results(self, trades: List[Trade], config: BacktestConfig) -> BacktestResult:
        """バックテスト結果を計算"""
        if not trades:
            return BacktestResult(config=config, start_date=config.start_date, end_date=config.end_date)
...
class BacktestResultExporter:
    ...
    def export_to_excel(self, result: BacktestResult, filename: Optional[str] = None) -> Path:
        """Excelファイルにエクスポート
        ...
        filepath = generate_timestamped_filename(filename.replace(".xlsx", ""), ".xlsx", self.output_dir)

        # サマリーデータを作成
        summary_data = {
            "項目": ["開始日", "終了日", "初期資金", "総利益", "総リターン", "取引回数", "勝率", "平均リターン", "シャープレシオ", "最大ドローダウン"],
            "値": [
        ...
        # Excelファイルに保存
        data_dict = {"サマリー": summary_df, "取引詳細": trades_df}
    ...
    def export_to_json(self, result: BacktestResult, filename: Optional[str] = None) -> Path:
        """JSONファイルにエクスポート
        ...
        filepath = generate_timestamped_filename(filename.replace(".json", ""), ".json", self.output_dir)
