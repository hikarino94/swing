...
from src.analysis.thresholds import SHORT_SIGNAL_COUNT_MIN, SIGNAL_COUNT_MIN, log_thresholds
...
def run_backtest(
    ...
    for idx, code in enumerate(sig_df["code"], start=1):
        ...
        try:
            ...
            prices = pd.read_sql(
                "SELECT date, adj_close AS close FROM prices " "WHERE code=? AND date>=? ORDER BY date",
                conn,
                params=(code, as_of),
                parse_dates=["date"],
            )
            ...
            try:
                r = conn.execute("SELECT company_name FROM listed_info WHERE code=?", (code,)).fetchone()
                name = r[0] if r else ""
...
if __name__ == "__main__":
    ...
    parser.add_argument("--stop-loss", type=float, default=STOP_LOSS_PCT_DEFAULT, help="損切り率")
    parser.add_argument(
        "--min-price",
        type=float,
