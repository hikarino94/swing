...
from src.analysis.screen_ml import (
    FUTURE_WINDOW,
    NUMERIC_STMT_COLS,
    PRICE_FEATURES,
    _add_label,
...
def _fetch_price_range(con: sqlite3.Connection, start: str, end: str) -> pd.DataFrame:
    q = "SELECT code, date, adj_close, adj_volume FROM prices " "WHERE date BETWEEN ? AND ?"
...
def run_backtest(
    ...
    train_end = (dt.datetime.strptime(start, "%Y-%m-%d") - dt.timedelta(days=1)).strftime("%Y-%m-%d")
